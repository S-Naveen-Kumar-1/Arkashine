...
export function BLEProvider({ children }) {
  ...
  const [sensorData, setSensorData] = useState({
    ec: null, ph: null, voltage: null,
    status: null, timer: null,
    raw: '', timestamp: 0, receivedCount: 0,
  });
  ...
  const parseData = useCallback((bytes) => {
    ...
    const base = { ec: null, ph: null, voltage: null, status: null, timer: null, raw, timestamp, receivedCount: dataCounterRef.current };
    ...
    try {
      ...
      addDebugLog('DATA', `JSON → EC=${parsed.ec}, pH=${parsed.ph}, V=${parsed.voltage}`);
      return parsed;
    } catch (_) {}
    ...
    for (const pair of raw.split(',')) {
      ...
      else if (key === 'voltage' || key === 'v') result.voltage = parseFloat(val);
      else if (key === 'status')               result.status  = val;
      else if (key === 'timer' || key === 'time') result.timer = parseInt(val, 10);
    }
    if (result.ec !== null || result.ph !== null) {
      addDebugLog('DATA', `KV → EC=${result.ec}, pH=${result.ph}, V=${result.voltage}`);
      return result;
    }
  ...
  // ─── Start notifications ─────────────────────────────────────────────────────
  const startNotifications = useCallback((device, cfg) => {
  ...
  const resolveConfig = useCallback(async (conn, currentMode) => {
    ...
    for (const svc of services) {
      ...
      for (const c of chars) {
        ...
        if (c.isWritableWithResponse || c.isWritableWithoutResponse) writeUUID = c.uuid;
      }
  ...
  const connect = useCallback(async (device) => {
    try {
      ...
      if (!cfg) {
        Alert.alert('UUID Error', 'Could not resolve UUIDs. Check UUID mode & values.');
      ...
      conn.onDisconnected(() => {
        ...
        notifySubRef.current?.remove();
        setSensorData({ ec: null, ph: null, voltage: null, status: null, timer: null, raw: '', timestamp: 0, receivedCount: 0 });

        reconnectRef.current = setTimeout(async () => {
          try {
            addDebugLog('RECONNECT', `Attempting reconnect (UUID mode: ${uuidMode})...`);
            const reconnected = await bleManager.connectToDevice(conn.id, { timeout: 8000 });
